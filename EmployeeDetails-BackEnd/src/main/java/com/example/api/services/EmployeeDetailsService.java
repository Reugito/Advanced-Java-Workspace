...
//					sentRegistrationConfirmationEmail(empDTO.emailId, empDTO.empId, empDTO.firstName);
	...
	public ResponseDTO findAllByStatusDetails(String status) {
		mongoClient.startSession().close();
		log.info("-----> "+ mongoClient.getCluster().isClosed());
...
//		log.info("/custerDes : "+mongoClient.getClusterDescription());
	 ...
	 * @param email
	 * @return Response DTO containing status, response code and employee details
	 *         that matches with email
	 */
	@Override
	public ResponseDTO findByEmail(String email) {
		log.info("/EmployeeDetailsService.findByEmail()");
		try {
			if(empRepo.countByEmail(email) !=0) {
				return new ResponseDTO("Succsess",200, empRepo.findByEachEntity("email", email));
			}else if(empRepo.countByEmail(email) ==0)
				return new ResponseDTO("Faild",400, "There no emp Details with this email");
		} catch (Exception e) {
			log.error("/error occured in EmployeeDetailsService.findByEmail()");
		}
		return new ResponseDTO("Faild",400, "Somthing went wrong");
	 ...
	 * @param address
	 * @return Response DTO containing status, response code and employee details
	 *         that matches with address
	 */
	@Override
	public ResponseDTO findByAddress(String address) {
		log.info("/EmployeeDetailsService.findByAddress()");
		System.out.println("sorted by :"+ empRepo.findAll(Sort.by("firstName")));
		try {
			if(empRepo.countByAddress(address) !=0) {
				return new ResponseDTO("Succsess",200, empRepo.findByEachEntity("address", address));
			}else if(empRepo.countByAddress(address) ==0)
				return new ResponseDTO("Faild",400, "There no emp Details with this address");
		} catch (Exception e) {
			log.error("/error occured in EmployeeDetailsService.findByAddress()");
		}
		return new ResponseDTO("Faild",400, "Somthing went wrong");
	...
	/**
	 * @apiNote this method is for sorting the employee details in ascending order
	 * 			according to Entity Name
	 * @param entityName
	 * @return Response DTO containing status, response code and sorted employee details
	 *         
	 */
	@Override
	public ResponseDTO sortByEachEntityAcending(String entityName) {
		log.info("/EmployeeDetailsService.sortByEachEntityAcending(String entityName)");
		try {
			return new ResponseDTO("succsess", 200, empRepo.findAll(Sort.by(entityName).ascending()));
		} catch (Exception e) {
			log.error("/error occured in EmployeeDetailsService.sortByEachEntityAcending(String entityName)");
		}
		return new ResponseDTO("failed", 400,"SomethingWent Wrong");
	}
	
	/**
	 * @apiNote this method is for sorting the employee details in descending order
	 * 			according to Entity Name
	 * @param entityName
	 * @return Response DTO containing status, response code and sorted employee details
	 *         
	 */
	public ResponseDTO sortByEachEntityInDescOrder(String entityName) {
		log.info("/EmployeeDetailsService.sortByEachEntityInDescOrder(String entityName)");
		try {
			return new ResponseDTO("succsess", 200, empRepo.findAll(Sort.by(entityName).descending()));
		} catch (Exception e) {
			log.error("/error occured in EmployeeDetailsService.sortByEachEntityInDescOrder(String entityName)");
		}
		return new ResponseDTO("failed", 400,"SomethingWent Wrong");
